...
CAST(NULL as varchar) as ReferenciaFornecedor, --varchar(30) --null
		...
		CASE 
			...
			WHEN(COALESCE(it."tipo",it2."tipo") = 'Lente Oftálmica' and scar2."Tipo fornitura dettaglio" = 2) THEN('LOD')
			WHEN(COALESCE(it."tipo",it2."tipo") = 'Lente Oftálmica' and scar2."Tipo fornitura dettaglio" = 3) THEN('LOE')
...
CAST(NULL as numeric(18,4)) as Diagonal, --numeric(18,4) --null
	...
	from busta as b
		...
		on ( (art."codice filiale" = scar2."codice articolo") or (art."codice a barre" = scar2."codice a barre") )

		full outer join articoli_fornitore as art_f
		on ( art."codice a barre" = art_f."codice a barre" )
		
		left join item as it
    	on ( it."CodigoAntigo" = 'articoli.'+ scar2."codice articolo" )

    	left join item as it2
    	on ( it2."CodigoAntigo" = 'articoli_fornitore.'+ scar2."codice a barre" )
